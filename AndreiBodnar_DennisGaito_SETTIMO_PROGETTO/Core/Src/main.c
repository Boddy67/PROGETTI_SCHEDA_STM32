...
/* USER CODE BEGIN PD */
/* USER CODE BEGIN PD */
// Indirizzo shiftato a sinistra (0x18 << 1 = 0x30 oppure 0x19 << 1 = 0x32)
// Proviamo 0x30 (SDO a GND). Se non va, cambia in 0x32 dopo.
#define LIS2DW12_ADDR      0x3
#define LIS2DW12_WHO_REG   0x0F
#define LIS2DW12_WHO_VAL   0x44
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
uint8_t sensor_id = 0;
HAL_StatusTypeDef ret_tx;
HAL_StatusTypeDef ret_rx;
uint8_t reg_to_read = LIS2DW12_WHO_REG;
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	HAL_StatusTypeDef ret; // Aggiungi questa variabile per il controllo
  /* USER CODE END 1 */
  ...
  // 1. Diciamo al sensore quale registro vogliamo leggere (Slide 29)
  ret_tx = HAL_I2C_Master_Transmit(&hi2c1, LIS2DW12_ADDR, &reg_to_read, 1, 100);

  // 2. Se la trasmissione è andata bene, leggiamo il risultato
  if (ret_tx == HAL_OK) {
      ret_rx = HAL_I2C_Master_Receive(&hi2c1, LIS2DW12_ADDR, &sensor_id, 1, 100);
  }
